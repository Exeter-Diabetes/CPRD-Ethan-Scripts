...
```{r Obtaining Baseline BMI, message=TRUE}
# Obtain all Patient IDs from T1T2 cohort that we need to calculate baseline BMI
...
for (i in 1:length(bmi_patid)) {
  ...
  upper_bound = 1:length(bmi_list)
  for (x in upper_bound){
   dates_and_differences[[bmi_list[x]]] = abs( difftime(bmi_list[x], date_diag, units="weeks") ) # Saves absolute value of differences between diag_date and BMI date
  }
  ...
  # Add a 2 week cut off to only calculate BMI 2-weeks before/after diagnosis 
  if (dates[1] > 4){
    t1t2_cohort = t1t2_cohort %>%
      mutate(bmi_date = ifelse(bmi_patid[i] == patid, NA, bmi_date)) %>%
      mutate(bmi = ifelse(bmi_patid[i] == patid, NA, bmi))
    next #close off this iteration of loop
  } 
  
  # If this secton has been reached, BMI date is <2 weeks
  for (x in upper_bound){
    if (dates_and_differences[[bmi_list[x]]] == dates[1]){
      bmi_diag_date = as.character(bmi_list[x])
    }
  } # This correctly saves the diagnosis date!
  
  bmi_value = bmi_data %>%
    filter(bmi_patid[i] == patid) %>%
    filter(date == bmi_diag_date)
  bmi_value = bmi_value$testvalue # this correctly saves the BMI value

...
t1t2_cohort = t1t2_cohort %>%
  ...
  analysis$cached("atdiagnosis_bmi_clean")
...
```{r Pushing to MySQL }
#Attempting Katie's way
t1t2_cohort = t1t2_cohort %>% 
  analysis$cached("t1t2_cohort") %>%
  mutate(BMI = NA) %>%
  analysis$cached('ethan_test')

data.frame(c("hello", 'test', 'testing'), c(1:3)) %>%
  analysis$cached('ethan_test_DF_cache')

---
title: "TSH Medcode list creation"
author: "Ethan de Villiers"
date: "`r Sys.Date()`"
output: html_document
---

```{r setup, echo=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(rio)
library(stringr)
library(dplyr)
library(tidyverse)
```

``` {r Preparing TSH data from PBCL}
# Importing PBCL dictionary
dictionary_path = '~/Downloads/CPRD_Read_SNOMED_PBCL.txt'
pbcl_dictionary <- import(dictionary_path)
View(pbcl_dictionary)

# Alter column datatypes
pbcl_dictionary = pbcl_dictionary %>%
  mutate(SNOMED_CONCEPTID = as.character(SNOMED_CONCEPTID))

# RegEx - Run and check results before continuing
( tsh_results_raw  = pbcl_dictionary %>%
  filter( 
    str_detect(V2_TERM, regex("TSH", ignore_case = TRUE)) == TRUE |
    str_detect(V3_TERM, regex("TSH", ignore_case = TRUE)) == TRUE |
    str_detect(SNOMED_TERM, regex("TSH", ignore_case = TRUE)) == TRUE ) )

#Building a cleaner dataframe
tsh_results = rbind.data.frame(
    tsh_results_raw %>% 
      select(V2_READ_CODE, V2_TERM) %>%
      mutate(Type = "V2_read") %>%
      rename("Codes" = "V2_READ_CODE") %>%
      rename("Terms" = "V2_TERM")
    ,
    tsh_results_raw %>% 
      select(V3_READ_CODE, V3_TERM) %>%
      mutate(Type = "V3_read") %>%
      rename("Codes" = "V3_READ_CODE") %>%
      rename("Terms" = "V3_TERM")
    ,
    tsh_results_raw %>% 
      select(SNOMED_CONCEPTID, SNOMED_TERM) %>%
      mutate(Type = "SNOMED") %>%
      rename("Codes" = "SNOMED_CONCEPTID") %>%
      rename("Terms"= "SNOMED_TERM")
  )

 
# Clean codelist
View(tsh_results)

# Removing duplicates
tsh_results = unique(tsh_results, by = "Codes")

# Removing unwanted codes
tsh_results = tsh_results %>%
  filter(Codes != "995841000000105" & Codes != "43Gu" & Codes != "54495009") %>% # Remove SNOMED TSH ~Antibody~ code
  filter(Codes != "408273009" & Codes != "1005531000000107") # Remove SNOMED TSH ~binding site inhibitors~
```


``` {r TSH Medcodes}
# Now can work with Code and Code_Type in tsh_results$
# Type = V2_read, V3_read, SNOMED

filepath = '~/Downloads/CPRDAurumMedical.txt'
Medcode_lookup <- import(filepath)

View(Medcode_lookup)

tsh_df = Medcode_lookup %>%
  mutate(read_code = substr(CleansedReadCode, 1, 5) ) %>%
  filter(read_code %in% tsh_results$Codes | SnomedCTConceptId %in% tsh_results$Codes) %>%
  select(MedCodeId, Term)

tsh_df = unique(tsh_df, by = "MedCodeId")

View(tsh_df)


```



``` {r Exporting data frames}
write.table(tsh_df, "exeter_medcodelist_tsh.txt", quote=FALSE, sep = "\t", row.names=FALSE, col.names=TRUE)
```